...
describe("remote MCP configuration", () => {
  test("requires an HTTPS public origin and password hash", () => {
    expect(() => loadRemoteMcpConfig({})).toThrow("TELESEND_MCP_PUBLIC_URL is required");
    expect(() =>
      loadRemoteMcpConfig({
        TELESEND_MCP_OWNER_PASSWORD_HASH: "$argon2id$valid",
  ...
  test("loads valid configuration without including the secret in errors", () => {
