...
describe("remote MCP configuration", () => {
  test("requires an HTTPS public origin and owner password hash", () => {
    ...
    ).toThrow("TELESEND_MCP_OWNER_PASSWORD_HASH is required");
  });

  test("does not accept a plaintext owner password", () => {
    expect(() =>
      loadRemoteMcpConfig({
